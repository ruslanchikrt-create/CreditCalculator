...
final class NavigationPanel {
    ...
    private static View header(Activity a,DrawerLayout drawer){
        ...
        String profileName=AppPreferences.getProfileName(a);String fio=AppPreferences.getCabinetFullName(a);String profile=!profileName.trim().isEmpty()?profileName:fio;String first=profile.trim().isEmpty()?"₽":profile.trim().substring(0,1).toUpperCase();fallback.setText(first);avatarBox.addView(fallback,new FrameLayout.LayoutParams(-1,-1));
        ...
        boolean hasProfile=!profile.trim().isEmpty();TextView main=new TextView(a);main.setText(hasProfile?profile:AppPreferences.tr(a,"Ваш профиль","Your profile"));main.setTextSize(hasProfile?18:19);main.setTextColor(Color.WHITE);main.setTypeface(null,android.graphics.Typeface.BOLD);main.setMaxLines(2);names.addView(main);
        TextView sub=new TextView(a);sub.setText(AppPreferences.tr(a,"Нажмите, чтобы изменить имя и аватар","Tap to edit name and avatar"));sub.setTextSize(12);sub.setTextColor(Color.argb(220,255,255,255));LinearLayout.LayoutParams sp=new LinearLayout.LayoutParams(-1,-2);sp.setMargins(0,dp(a,3),0,0);names.addView(sub,sp);
        h.setClickable(true);h.setFocusable(true);h.setOnClickListener(v->{drawer.closeDrawer(GravityCompat.START);if(!(a instanceof ProfileActivity))a.startActivity(new Intent(a,ProfileActivity.class));});return h;
    }
