...
public class AddReminderActivity extends AppCompatActivity {
    ...
    private void addTermRow(LinearLayout p){LinearLayout row=new LinearLayout(this);row.setOrientation(LinearLayout.HORIZONTAL);row.setGravity(Gravity.CENTER_VERTICAL);LinearLayout.LayoutParams rp=new LinearLayout.LayoutParams(-1,-2);rp.setMargins(0,0,0,dp(12));p.addView(row,rp);termLayout=createLayout(AppPreferences.tr(this,"Срок","Term"));row.addView(termLayout,new LinearLayout.LayoutParams(0,-2,1f));termInput=createInput(InputType.TYPE_CLASS_NUMBER);termLayout.addView(termInput,new TextInputLayout.LayoutParams(-1,-2));termUnitSpinner=createSpinner();LinearLayout.LayoutParams up=new LinearLayout.LayoutParams(dp(136),dp(58));up.setMargins(dp(8),0,0,0);row.addView(termUnitSpinner,up);}
