...
public class PatternLockView extends View {
    ...
    public PatternLockView(Context context, @Nullable AttributeSet attrs) { super(context, attrs); init(); }
    public PatternLockView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) { super(context, attrs, defStyleAttr); init(); }

    private void init() {
        setFocusable(true);
        setClickable(true);

        // Unselected dots must stay clearly visible on both the app card and Android dialog backgrounds.
        dotPaint.setStyle(Paint.Style.FILL);
        dotPaint.setColor(ContextCompat.getColor(getContext(), R.color.primary));
        dotPaint.setAlpha(210);
        dotRingPaint.setStyle(Paint.Style.STROKE);
        dotRingPaint.setStrokeWidth(dp(3));
        dotRingPaint.setColor(Color.WHITE);
        dotRingPaint.setAlpha(190);
        ...
        linePaint.setStrokeWidth(dp(6));
        linePaint.setColor(ContextCompat.getColor(getContext(), R.color.primary));
        linePaint.setAlpha(255);
        setMinimumHeight(Math.round(dp(250)));
    }

    public void setOnPatternCompleteListener(OnPatternCompleteListener listener) { this.listener = listener; }
    public void reset() { selected.clear(); drawing = false; error = false; invalidate(); }
    public void showError() { error = true; drawing = false; invalidate(); postDelayed(this::reset, 700); }
    ...
    @Override protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        float step = size / 3f;
        dotRadius = Math.max(dp(10), Math.min(dp(17), step * 0.13f));
        hitRadius = Math.max(dp(34), step * 0.38f);
    ...
    @Override protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            if (active) {
                canvas.drawCircle(xs[i], ys[i], dotRadius * 1.55f, selectedPaint);
                canvas.drawCircle(xs[i], ys[i], dotRadius * 0.55f, innerPaint);
            } else {
                canvas.drawCircle(xs[i], ys[i], dotRadius * 1.32f, dotRingPaint);
    ...
    @Override public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                if (!selected.isEmpty()) reset(); drawing = true; addHitNode(currentX, currentY); invalidate(); return true;
            case MotionEvent.ACTION_MOVE:
                addHitNode(currentX, currentY); invalidate(); return true;
            case MotionEvent.ACTION_UP:
                drawing = false; addHitNode(currentX, currentY); invalidate(); performClick();
                if (listener != null) listener.onPatternComplete(getPatternString()); return true;
    ...
    private void addHitNode(float x, float y) {
        ...
        if (!selected.isEmpty()) {
            int middle = middleNode(selected.get(selected.size() - 1), hit);
    ...
    private int hitTest(float x, float y) {
        float best = Float.MAX_VALUE; int result = -1;
        for (int i = 0; i < 9; i++) {
            float dx = x - xs[i], dy = y - ys[i], d2 = dx * dx + dy * dy;
            if (d2 <= hitRadius * hitRadius && d2 < best) { best = d2; result = i; }
        }
        return result;
    }
